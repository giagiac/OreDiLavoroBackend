...
@Injectable()
export class X1TrasCodiciRelationalRepository
  implements X1TrasCodiciRepository
{
  constructor(
  ...
  async create(data: X1TrasCodici): Promise<X1TrasCodici> {
    ...
    const newEntity = await this.x1TrasCodiciRepository.save(
      this.x1TrasCodiciRepository.create(persistenceModel),
    );
  ...
  ): Promise<NullableType<X1TrasCodici>> {
