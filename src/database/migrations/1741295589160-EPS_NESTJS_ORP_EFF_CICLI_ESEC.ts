...
"FLAG_M_DFI" NUMBER(3,0))`,
    ...
    await queryRunner.query(
      `COMMENT ON COLUMN EPS_NESTJS_ORP_EFF_CICLI_ESEC."id" IS 'Identificativo univoco della riga'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN EPS_NESTJS_ORP_EFF_CICLI_ESEC."createdAt" IS 'Data e ora di creazione della riga'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN EPS_NESTJS_ORP_EFF_CICLI_ESEC."updatedAt" IS 'Data e ora dell''ultimo aggiornamento'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN EPS_NESTJS_ORP_EFF_CICLI_ESEC."deletedAt" IS 'Data e ora di eliminazione logica (soft delete)'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN EPS_NESTJS_ORP_EFF_CICLI_ESEC."SYNCED" IS 'Flag per indicare se i dati sono sincronizzati'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN EPS_NESTJS_ORP_EFF_CICLI_ESEC."TIPO_TRASFERTA" IS 'Tipo di trasferta associata'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN EPS_NESTJS_ORP_EFF_CICLI_ESEC."KM" IS 'Chilometri percorsi'`,
    );
