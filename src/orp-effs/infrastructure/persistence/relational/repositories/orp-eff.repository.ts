...
export class OrpEffRelationalRepository implements OrpEffRepository {
  ...
  async create(data: OrpEff): Promise<OrpEff> {
    ...
    const newEntity = await this.orpEffRepository.save(this.orpEffRepository.create(persistenceModel));
    return OrpEffMapper.toDomain(newEntity);
  }

  async findAllWithPagination({ paginationOptions }: { paginationOptions: IPaginationOptions }): Promise<OrpEff[]> {
  ...
  async update(DOC_ID: OrpEff['DOC_ID'], payload: Partial<OrpEff>): Promise<OrpEff> {
    const entity = await this.orpEffRepository.findOne({
      where: { DOC_ID },
    });
